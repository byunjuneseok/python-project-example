...
async def auth_api(
    ...
    if roles and result.role not in roles:
        raise UnauthorizedException(code=AuthenticationCodeEnum.INVALID_TOKEN, message="사용자 토큰이 아닙니다.")
    # set_user({"id": result.identifier})  # sentry
    return Auth(user_id=result.identifier, role=result.role, token_payload=result)


async def auth_user_api(request: Request, token: HTTPAuthorizationCredentials = Depends(HTTPAPIBearer())):
    return await auth_api(request, roles={"user"}, token=token, scope_to_verify="core-api:v1:user")


async def auth_owner_api(request: Request, token: HTTPAuthorizationCredentials = Depends(HTTPAPIBearer())):
    return await auth_api(request, roles={"owner"}, token=token, scope_to_verify="core-api:v1:owner")
